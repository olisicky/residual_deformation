...
########################## Definice hlavního okna #############################
    def init_window(self):
        ...
        self.Frame1=Frame(self, width=Des_Width/3, height=Des_Height/2,bg="red")
        self.Frame1.grid(row = 0, column = 0,rowspan= 2, columnspan=1, sticky=(N, S, W, E))
        self.Frame2 = Frame (self, width=2 * Des_Width/3, height=2*Des_Height/3, bg="blue")
        self.Frame2.grid(row = 0, column = 1, rowspan= 3, columnspan=2, sticky=(N, S, W, E))
       
        self.Frame3 = LabelFrame (self, width=Des_Width/3, height=Des_Height/2, bg="green")
        self.Frame3.grid(row = 2, column = 0, rowspan=3, sticky=(S))
        self.Frame4 = Frame (self, width=Des_Width/3, height=Des_Height/3, bg="purple")
        self.Frame4.grid(row = 3, column = 1, sticky=(N, S, W, E))
        self.Frame5 = Frame (self, width=Des_Width/3, height=Des_Height/3, bg="black")
        self.Frame5.grid(row = 3, column = 2, sticky=(N, S, W, E))
...
###########################Přidání obrázku po kliknutí#########################
        self.ImgButt = Button(self.Frame1, text = "Otevřít obrázek", command = self.openImage)
        self.ImgButt.place(x=1, y=1)
############################### Vykreslení grafu###############################
        self.GraphButt = Button(self.Frame2, text = "Vykresli graf", command = self.drawGraph)
        self.GraphButt.place(x=1, y=1)
        
###############################   Import image    #############################
        self.ImImport = Button(self.Frame2, text = "Nahrát snímek", command = self.importImage)
        self.ImImport.place(x=100, y=100)
...
root.geometry("%dx%d+0+0" % (Des_Width, Des_Height))    #Takhle jsem schopen vložit něco do string!
app = Window(root)
